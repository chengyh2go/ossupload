...
func init()  {
	flag.BoolVar(&Help, "help", false, "oss断点续传客户端使用帮助")
	flag.StringVar(&OssEndpoint, "ossEndpoint",
		"","输入OSS的Endpoint")
	flag.StringVar(&OssAccessKeyID, "ossAccessKeyID", "","输入accessKeyID")
	flag.StringVar(&OssAccessKeySecret, "ossAccessKeySecret", "","输入accessKeySecret")
	flag.StringVar(&OssBucketName, "ossBucketName", "","输入bucket name")
	flag.StringVar(&OssObjectPrefix, "ossObjectPrefix", "","输入object前缀,注意以'/'结尾，比如TD/h5/")
	flag.StringVar(&OssObjectName, "ossObjectName", "","输入object name,如果不输入，默认就是上传的文件名")
...
func usage() {
	_, _ = fmt.Fprintf(os.Stdout, `oss断点续传客户端: v1.0
Options:
`)
	flag.PrintDefaults()
}
...
func main() {
	...
	if Help {
		...
		//_ = client.SetBucketACL(OssBucketName,oss.ACLPublicReadWrite)
